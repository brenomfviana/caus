...
size_t** CellularAutomata::get_neighborhood(World* world_) {
...
void CellularAutomata::birth_rule(size_t** wn_, World* world_) {
...
void CellularAutomata::death_rule(size_t** wn_, World* world_) {
  for (size_t i = 0; i < world_->get_width(); i++) {
    for (size_t j = 0; j < world_->get_height(); j++) {
...
void CellularAutomata::apply_ca(World* world_) {
  size_t** wn = get_neighborhood(world_);
